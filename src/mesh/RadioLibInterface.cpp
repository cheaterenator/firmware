...
The CW size is determined by setTransmitDelay() and depends either on the current channel utilization, for a packet we
originated, or on the SNR, for one we are relaying - which of the two applies is decided by provenance, see
RadioInterface::isLocallyOriginated(). After this, we perform channel activity detection (CAD) and reset the transmit
delay if it is currently active.
*/
...
void RadioLibInterface::setTransmitDelay()
{
    ...
    if (p->tx_after) {
        unsigned long add_delay = isLocallyOriginated(p) ? getTxDelayMsec() : getTxDelayMsecWeighted(p);
    ...
    } else if (isLocallyOriginated(p)) {
        // Our own packet, so nothing to defer to: just the plain channel-utilization backoff.
        startTransmitTimer(true);
    } else {
        // Relaying another node's packet, whatever transport carried it to us. Back off on the
        // SNR-weighted scale so we stay behind the originator - see isLocallyOriginated().
        LOG_TRACE("Relay tx delay. hop_limit:%d rx_snr:%f transport:%u", p->hop_limit, p->rx_snr, p->transport_mechanism);

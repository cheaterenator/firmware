...
ProcessMessage SignalReplyModule::handleReceived(const meshtastic_MeshPacket &currentRequest)
{
    ...
    if ( ( receivedMessage  == "pingunr" || receivedMessage  == "Pingunr" ) && currentRequest.from != 0x0 && currentRequest.from != nodeDB->getNodeNum())
    {
        ...
        const char *username = (nodeSender && nodeInfoLiteHasUser(nodeSender)) ? nodeSender->short_name : idSender;
        meshtastic_NodeInfoLite *nodeReceiver = nodeDB->getMeshNode(nodeDB->getNodeNum());
        const char *usernameja = (nodeReceiver && nodeInfoLiteHasUser(nodeReceiver)) ? nodeReceiver->short_name : idReceipient;
        ...
        {
            snprintf(messageReply, sizeof(messageReply), "%s: odebrane poprzez %d wezly(ow), nie podam SNR / RSSI", username, (hopsUsed));
        }
        else
        {

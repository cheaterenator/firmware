...
void SimRadio::setTransmitDelay()
{
    ...
    // Provenance, not the RF fields, picks the window - see RadioInterface::isLocallyOriginated().
    if (isLocallyOriginated(p)) {
        startTransmitTimer(true);
    } else {
        // Relaying another node's packet: back off on the SNR-weighted scale so we stay behind the originator.
